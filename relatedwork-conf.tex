...
\emph{Update Procedure}: When an update $(i_{t},c_{t})$ arrives, meaning that item $a_{i_{t}}$is updated by a quantity of $c_{t}$ , then $c_{t}$ is added to one count in each row; the counter is determined by$h_{j}$. Formally, set $\forall 1 \leq j \leq d: count[j,h_{j}(i_{t})]\leftarrow count[j,h_{j}(i_{t})] + c_{t}$. The space used by Count-Min sketches is the two-dimensional array and $d$ hash functions.
